...
public final class AlamofireManager: NetworkProtocol {
    ...
    public func call<T, U>(api: U, model: T.Type) -> RSResponse<T> where T : Decodable, T : Encodable, U : Endpoint {
...
public struct OErrorParser {
    ...
    public func parse(_ error: Error) -> OError {
        switch error {
...
//            return GIDSignInErrorToOErrorAdapter().adapt(googleSignInError)
        ...
        case let validationError as ValidationError:
                return .init(text: validationError.errorDescription ?? "something went wrong")
        default:
...
public extension Error {
    public func toOError() -> OError {
        OErrorParser().parse(self)
    }
}
